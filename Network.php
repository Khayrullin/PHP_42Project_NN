    ...
    class Network
{
    ...
    public function train(Network $net) {
        ...
        do {
            ...
            echo "Learning error " . $temp_cost . " in epoch " . $epoches . "\n";
            $epoches++;
    ...
    public function test(Network $net) {
        for ($i = 0; $i < $net->input_layer->getCountTestset(); $i++) {
            ...
            print_r("Source data for recognition: ");
            print_r($net->input_layer->getTestset($i)[1]);
            print_r("\n");
            for ($k = 0; $k < 5; $k++) {
                for ($l = 0; $l < 3; $l++) {
                    print($net->input_layer->getTestset($i)[0][$l + $k * 3]);
                }
                print_r("\n");
            }
            print_r("\n");
            print_r("Output data of neural network:\n");
            $recogn = 0;
            for ($j = 0; $j < count($net->fact); $j++) {
                if ($net->fact[$j] > $net->fact[$recogn]) {
                    $recogn = $j;
                }
                echo $net->fact[$j] . "\n";
            }
            echo "It's probably " . $recogn . "\n";
            echo "\n";
        }
    }
